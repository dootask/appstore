...
这是 [DooTask](https://github.com/kuaifan/dootask) 的官方应用商店仓库，包含所有可通过 [DooTask](https://github.com/kuaifan/dootask) 应用商店安装的应用程序。
...
# 版本卸载要求（可选）
require_uninstalls:               # 指定哪些版本需要先卸载
  - version: "2.0.0"              # 需要卸载的特定版本
    reason: "Structure changes"   # 卸载原因（支持多语言）
...
# 入口点配置（可选）
entry_points:                     # 定义应用菜单入口
  - location: application         # 入口位置（支持的值：application, application/admin, main/menu）
    label:                        # 入口标签（支持多语言）
      en: App Management
      zh: 应用管理
    url: apps/appstore/list       # 入口 URL
    icon: ./icon.png              # 入口图标路径
...
*注意：此配置将通过 include 指令包含在现有的服务器块中。无需定义新的服务器块。* 